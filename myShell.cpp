...
static int if_state = Netural;
static int execution_state = Regular;
static int else_flag = 0;

char statment_command[1024];
char true_command[1024];
char false_command[1024];

int is_controlflow_start(char *s[])
{
    return (strcmp(s[0], "if") == 0 && strcmp(s[i - 1], "fi") == 0);
}
...
/**
 * we will get in to this func if we saw and fi
 */
void if_handler(char *arg[])
{

    int t = 0;
    strcpy(statment_command, "");
    strcpy(true_command, "");
    strcpy(false_command, "");

    while (arg[t] != NULL)
    {
        if (is_control_command(arg[t]))
        {
            if (!strcmp(arg[t], "if") && if_state == Netural)
            {
                if_state = Want_then;
            }
            else if (!strcmp(arg[t], "then") && if_state == Want_then)
            {
                if_state = Then_block;
            }
            else if (!strcmp(arg[t], "else") && if_state == Then_block)
            {
                if_state = else_block;
                else_flag = 1;
            }
            else if (!strcmp(arg[t], "fi") && if_state == else_block)
            {
                if_state = Netural;
                execution_state = if_statment;
            }
        }
        else
        {
            ...
            {
                strcat(statment_command, arg[t]);
                strcat(statment_command, " ");
                strcat(statment_command, "\0");
            }
            if(if_state == Then_block)
            {
                strcat(true_command, arg[t]);
                strcat(true_command, " ");
                strcat(true_command, "\0");  
            }
            if(if_state == else_block)
            {
                strcat(false_command, arg[t]);
                strcat(false_command, " ");
                strcat(false_command, "\0");
            }
        }
        t++;
    }
...
int main()
{
    ...
    while (1)
    {
        ...
        else if(execution_state == if_statment){
            strcpy(command,statment_command);
            execution_state = T_command;//inform the system that next one we trying to output the if resualt
        }
        else if(execution_state == T_command)
        {
            if(!status)//execution were ok
            {
                strcpy(command,true_command);    
                execution_state = Regular;
            }
            else if(else_flag)
            {
                strcpy(command,false_command);     
                execution_state = Regular;
            }
            execution_state = Regular;
        }
        ...
        { // added the left cuse can't comper null
            redirect_err = 1;
            argv[i - 2] = NULL;
            outfile = argv[i - 1];
        }
